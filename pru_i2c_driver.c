...
uint8_t pru_i2c_driver_WaitXRDY()
{
    uint32_t ticks = 0;
...
uint8_t pru_i2c_driver_WaitRRDY()
{
    uint32_t ticks = 0;
...
uint8_t pru_i2c_driver_WaitARDY()
{
    uint32_t ticks = 0;
...
uint8_t pru_i2c_driver_WaitRDONE()
{
    uint32_t ticks = 0;
...
{
    return pru_i2c_driver_ReadBytes(address, reg, 1, buffer);
}
uint8_t pru_i2c_driver_WriteReg(uint8_t address, uint8_t reg, uint8_t value)
{
    return pru_i2c_driver_WriteBytes(address, reg, 1, &value);
}
...
uint8_t pru_i2c_driver_Init() {
    ...
    CT_I2C2.I2C_SYSC_bit.I2C_SYSC_CLKACTIVITY = 0b00; // cutoff interface ocp clock and functional system clock

    pru_i2c_driver_Set400KHz();

    // I2C_BUF as default: DMA disabled and buffer tx/rx lenght = 1

    // enable i2c2
    CT_I2C2.I2C_CON_bit.I2C_CON_I2C_EN = 0b1;
    if(pru_i2c_driver_WaitRDONE()) {
        return 1;
    }
    return 0;
}
